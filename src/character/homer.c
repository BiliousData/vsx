...
static SkullFragment char_homer_skull[15] = {
	{ 1 * 8, -87 * 8, -13, -13},
	{ 9 * 8, -88 * 8,   5, -22},
	{18 * 8, -87 * 8,   9, -22},
	{26 * 8, -85 * 8,  13, -13},
...
enum
{
	Homer_ArcDead_Dead1, //Mic Drop
	Homer_ArcDead_Dead2, //Twitch
	Homer_ArcDead_Retry, //Retry prompt
	
	Homer_ArcDead_Max,
};

#define Homer_Arc_Max Homer_ArcMain_Max
...
typedef struct
{
	...
	SkullFragment skull[COUNT_OF(char_homer_skull)];
	u8 skull_scale;
} Char_Homer;
...
void Char_Homer_Tick(Character *character)
{
	...
	if (character->animatable.anim >= PlayerAnim_Dead3)
	{
		...
		if (this->skull_scale)
		{
			...
			for (size_t i = 0; i < COUNT_OF_MEMBER(Char_Homer, skull); i++, frag++)
			{
				//Draw fragment
				RECT frag_src = {
					(i & 1) ? 112 : 96,
	...
	switch (anim)
	{
		...
		case PlayerAnim_Dead2:
			...
			const char **pathp = (const char *[]){
				"dead1.tim", //Homer_ArcDead_Dead1
				"dead2.tim", //Homer_ArcDead_Dead2
				"retry.tim", //Homer_ArcDead_Retry
				NULL
			};
			...
			//Load retry art
			Gfx_LoadTex(&this->tex_retry, this->arc_ptr[Homer_ArcDead_Retry], 0);
			break;
	}
...
Character *Char_Homer_New(fixed_t x, fixed_t y)
{
	...
	if (this == NULL)
	{
		sprintf(error_msg, "[Char_Homer_New] Failed to allocate boyfriend object");
		ErrorLock();
		return NULL;
	}
	...
	const char **pathp = (const char *[]){
		"idle.tim",  //Homer_ArcMain_Idle
		"hit0.tim",  //Homer_ArcMain_Hit0
		"miss0.tim", //Homer_ArcMain_Miss0
		"hit1.tim",  //Homer_ArcMain_Hit1
		"miss1.tim", //Homer_ArcMain_Miss1
		"peace.tim", //Homer_ArcMain_Peace
		"dead0.tim", //Homer_ArcMain_Dead0
		NULL
	};
	IO_Data *arc_ptr = this->arc_ptr;
	for (; *pathp != NULL; pathp++)
	...
	memcpy(this->skull, char_homer_skull, sizeof(char_homer_skull));
	this->skull_scale = 64;
